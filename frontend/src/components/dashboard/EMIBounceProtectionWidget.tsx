    ...
    <div className="card p-5 border border-[var(--color-border)] space-y-4 bg-[var(--color-surface)]">
      <div className="flex items-center justify-between border-b border-[var(--color-border)] pb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-[var(--color-text-secondary)] font-medium">Prevent ₹500+ EMI bounce charges & NACH auto-debit failure penalties</p>
          </div>
        </div>
        <span className={`text-xs font-black ${risk.color}`}>{risk.label}</span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
        <div>
          ...
          <div className="p-2.5 rounded-xl bg-[var(--color-surface-secondary)] border border-[var(--color-border)] font-bold text-[var(--color-primary)] text-sm">
        ...
        <div>
          ...
          <div className={`p-2.5 rounded-xl border font-bold text-xs flex items-center justify-between ${risk.bg}`}>
            <span className="text-[var(--color-text-primary)]">{risk.badge}</span>
            <span className={`font-black ${netShortfall > 0 ? "text-rose-700 dark:text-rose-400" : "text-emerald-700 dark:text-emerald-400"}`}>
