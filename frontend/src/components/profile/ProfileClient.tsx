...
export function ProfileClient() {
  ...
  const { currentPlan, openPaywall } = useSubscription();
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full bg-rose-500/10 text-rose-500 border border-rose-500/30">
                {currentPlan?.name || "FREE PLAN"} Member
              </span>
            </div>
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-6 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <h3 className="text-lg font-black text-[var(--color-text-primary)]">Current Tier: <span className="text-rose-500 uppercase">{currentPlan?.name || "FREE PLAN"}</span></h3>
        ...
        <button
          onClick={() => openPaywall({ reason: "Manage or upgrade your DebtProof membership tier to unlock all pro features." })}
